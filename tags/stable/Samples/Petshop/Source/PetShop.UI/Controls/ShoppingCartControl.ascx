<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ShoppingCartControl.ascx.cs" Inherits="PetShop.UI.Controls.ShoppingCartControl" %>
...
<asp:Panel ID="panFocus" runat="server" DefaultButton="btnTotal">
    ...
    <asp:Repeater ID="repShoppingCart" runat="server">
        ...
        <ItemTemplate>
            <tr class="listItem">
                ...
                <td style="width: 100%;">
                    <a id="A1" runat="server" href='<%# string.Format("~/Items.aspx?itemId={0}&productId={1}&categoryId={2}", Eval("ItemId"), Eval("ProductId"), Eval("CategoryId")) %>'>
                        <%# string.Format("{0} {1}", Eval("Name"), Eval("Type")) %></a>
                </td>
                <td>
                    <asp:TextBox ID="txtQuantity" runat="server" Columns="3" Text='<%# Eval("Quantity") %>'
                ...
                <td>
                    <asp:ImageButton ID="btnToWishList" runat="server" AlternateText="Move to wish list"
                        CausesValidation="false" CommandArgument='<%# Eval("ItemId") %>' CommandName="Move"
                        ImageUrl="~/Comm_Images/button-wishlist.gif" OnCommand="CartItem_Command" ToolTip="Move to wish list" />
                </td>
            </tr>
        </ItemTemplate>
        <FooterTemplate>
    ...
    <asp:PlaceHolder ID="plhTotal" runat="server" EnableViewState="false">
        <table border="0" cellpadding="0" cellspacing="0" width="387">
            <tr>
                <td class="dottedLineCentered" colspan="4">
                    &nbsp;
                </td>
            </tr>
            <tr>
                <td class="total" width="30">
                    Total
                </td>
                <td>
                    <asp:ImageButton ID="btnTotal" runat="server" AlternateText="Calculate total" CausesValidation="false"
                        ImageUrl="~/Comm_Images/button-calculate.gif" OnClick="BtnTotal_Click" EnableViewState="false" />
                </td>
