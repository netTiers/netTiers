...
public class PetShopSiteMapProvider : StaticSiteMapProvider
{
    ...
    //private Dictionary<string, SiteMapNode> _parentHash = new Dictionary<string, SiteMapNode>(16);
    ...
    public override SiteMapNode BuildSiteMap()
    {
        lock (this)
        {
            ...
            TList<Item> items = DataRepository.ItemProvider.GetAll();

            foreach (Category category in categories)
            {
                SiteMapNode categoryNode = CreateSiteMapNodeFromCategory(category);
                products.Filter = string.Format("CategoryId = '{0}'", category.Id);
                category.ProductCollection = products; //.FindAll(SerialCoder.PetShop.ProductColumn.CategoryId, category.Id);
                foreach (Product product in category.ProductCollection)
                {
                    ...
                    items.Filter = string.Format("ProductId = '{0}'", product.Id);
                    product.ItemCollection = items;
                    //product.ItemCollection = items.FindAll(SerialCoder.PetShop.ItemColumn.ProductId, product.Id);
                ...
                //HttpContext.Current.Response.Write(category.Name + "<br>");
	...
	private SiteMapNode CreateSiteMapNodeFromItem(netTiers.PetShop.Item item)
    {
        // Make sure the node ID is unique
