	...
	public class CommonSqlCode : CodeTemplate
	{
		...
		public string GetObjectPropertySetExpression(ColumnSchema column, string containerName, string objectName, int indent)
		{
			if ( column.AllowDBNull )
			{
				...
				{
					// nullable value types call SetColumnNameNull() function
					return string.Format(@"if ( Convert.IsDBNull({1}[""{0}""]) )
{7}{5}.Set{2}Null();
{6}else
{7}{3} = ({4}){1}[""{0}""]", 
						/*0*/column.Name,
						/*1*/containerName,
						/*2*/GetPropertyName(column.Name),
						/*3*/GetObjectPropertyAccessor(column,objectName),
						/*4*/GetCSType(column),
						/*5*/objectName,
						/*6*/Tab(indent),
						/*7*/Tab(indent+1));
...
{7}{3} = ({4}){1}[""{0}""]", 
		...
		public string TransformStoredProcedureInputsToMethod(bool startWithComa, ParameterSchemaCollection inputParameters)
		{
			...
			for(int i=0; i<inputParameters.Count; i++)
			{
				if ((i>0) || startWithComa)
					temp.Append(", ");
				//comment out by john teague, ben johnson has a solution for output paramteres
				//in 2.0 templates.  will integrate those changes with theses
				//3/3/2006
				//if(inputParameters[i].Direction != ParameterDirection.Input)
				//{
				//	temp.Append("out ");
				//}	
		...
		public string TransformStoredProcedureInputsToMethodComments(ParameterSchemaCollection inputParameters)
		{
			...
			for(int i=0; i<inputParameters.Count; i++)
			{
				temp.AppendFormat("{2}\t/// <param name=\"{0}\"> A <c>{1}</c> instance.</param>", GetPrivateName(inputParameters[i].Name.Substring(1)), GetCSType(inputParameters[i]), Environment.NewLine);
			}
		...
		public ArrayList GetChildrenCollections(SchemaExplorer.TableSchema table, SchemaExplorer.TableSchemaCollection tables) 
		{
			...
			foreach(TableKeySchema keyschema in primaryKeyCollection)
			{
				...
				if(IsRelationOneToOne(keyschema))
				{
					...
					collectionInfo.CleanName = GetClassName(keyschema.ForeignKeyTable.Name);//GetClassName(keyschema.ForeignKeyTable.Name);		
				...
				if ( tables.Contains(key.ForeignKeyTable.Owner, key.ForeignKeyTable.Name) &&  IsJunctionTable(key.ForeignKeyTable) && IsJunctionKey(key))
				{
					...
					{				
						if ( tables.Contains(junctionTableKey.ForeignKeyTable.Owner, junctionTableKey.ForeignKeyTable.Name) && IsJunctionKey(junctionTableKey) && junctionTableKey.Name != key.Name )
						{
							...
							collectionInfo.CollectionName = string.Format("{0}_From_{1}", GetCollectionClassName( collectionInfo.SecondaryTable), GetClassName(collectionInfo.JunctionTable)); //GetManyToManyName(GetCollectionClassName( collectionInfo.SecondaryTable), collectionInfo.JunctionTable);
...
*/
		...
		*/
		...
		///<summary>
		/// returns the correct return type for custom procedures for a table.  Return values will be either "void",collectionClassName, or "DataSet"
		///</summary>
		public string CustomMethodReturnType(CommandSchema command, string collectionClassName, SchemaExplorer.TableSchema SourceTable){
			string returnType = string.Empty;
			if(command.CommandResults == null || command.CommandResults.Count == 0)
			{
			     returnType = "void";
			}
			else if (IsMatching(command, SourceTable))
			{
		...
		///<summary>
		/// returns the correct return type for custom procedures for a view.  Return values will be either "void",collectionClassName, or "DataSet"
		///</summary>
		public string CustomMethodReturnType(CommandSchema command, string collectionClassName, SchemaExplorer.ViewSchema SourceView){
			string returnType = string.Empty;
			if(command.CommandResults == null || command.CommandResults.Count == 0)
			{
			     returnType = "void";
			}
			else if (IsMatching(command, SourceView))
			{
				returnType = collectionClassName;
			}
			else	// Return a basic DataSet
