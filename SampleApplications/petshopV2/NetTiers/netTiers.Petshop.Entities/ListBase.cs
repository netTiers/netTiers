...
namespace netTiers.Petshop.Entities
{
   ...
   public abstract class ListBase<T> : BindingList<T>, IBindingListView, IBindingList, IList, ICloneable, IListSource, ITypedList, IDisposable, IComponent, IRaiseItemChangedEvents, IDeserializationCallback
   {
      ...
      private void InitializeList()
      {
		...
		//TODO: Planned on removing 
		/* 
		foreach (PropertyDescriptor propDesc in _propertyCollection) 
		 {
            if (propDesc.SupportsChangeEvents)
            {
               supportsChangeEventsProperties.Add(propDesc);
            }
         }
		*/
	...
	*/
      ...
      */
      ...
      #region ToDataSet
      /// <summary>
      /// NOTE: EXPERIMENTAL CODE, Please do not use this in a production environment.  
      /// Need to refactor to a few methods, secure it down a bit, and do more type checking.
      ...
      #region AddRelations
      /// <summary>
      /// NOTE: EXPERIMENTAL CODE, Please do not use this in a production environment.  Need to refactor to a few methods, secure it down a bit, and do more type checking.
